...
import { getGitHubStatus } from './githubPoller.js';
...
export function getPulseData(req, res) {
  ...
  const filteredEvents = state.events.filter(e => new Date(e.timestamp).getTime() > cutoff);
  ...
  const stats = {
    ...
    activeCountries: new Set(filteredEvents.filter(e => e.location?.country).map(e => e.location.country)).size,
  };
  ...
  filteredEvents.forEach(e => {
    if (!repoCounts[e.repository]) {
      repoCounts[e.repository] = { name: e.repository, count: 0, latestActivity: e.timestamp };
    }
