...
async function pollGitHub() {
  if (status.status === "rate_limited" && status.rateLimitReset) {
    ...
    if (now < status.rateLimitReset) {
      console.log(
        `[Poller] Waiting for rate limit reset at ${status.rateLimitReset}`,
      );
      return;
    } else {
      status.status = "ok";
  ...
  try {
    ...
    if (res.status === 403 || res.status === 429) {
      console.warn(
        `[Poller] GitHub API Rate Limited. Remaining: ${status.rateLimitRemaining}`,
      );
    ...
    for (const event of events) {
      ...
      if (locationStr === undefined && process.env.GITHUB_TOKEN) {
        try {
          const userRes = await fetch(`https://api.github.com/users/${actor}`, {
            headers,
          });
          if (userRes.ok) {
